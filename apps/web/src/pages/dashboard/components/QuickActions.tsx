...
const actions = [
  { label: 'Nový záznam', icon: '📝', path: '/evidence/new', gradient: 'from-indigo-500 to-violet-500' },
  { label: 'Evidence', icon: '📋', path: '/evidence', gradient: 'from-emerald-500 to-teal-500' },
  { label: 'Stroje', icon: '⚙️', path: '/machines', gradient: 'from-amber-500 to-orange-500' },
  { label: 'Uživatelé', icon: '👥', path: '/admin/users', gradient: 'from-sky-500 to-blue-500' },
];
    ...
    <GlassCard hover={false} className="animate-fade-in-up" style={{ animationDelay: '400ms' }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {actions.map((action, i) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-r ${action.gradient} flex items-center justify-center text-lg shadow-md`}>
              {action.icon}
            </div>
